...
class OpenTag(object):
    def __init__(self, tag, coalesce=False, attributes=None, self_closes=False):
        self.tag = tag
    ...
    def __attribute_string(self):
        ...
        for key in self.attributes:
            result += ' %s="%s"' % (
                key,
                str(self.attributes[key]).replace("&", "&amp;").replace('"', "&quot;"),
            )
        return result
    ...
    def __repr__(self):
        return "<OpenTag %s %s>" % (self.tag, self.__attribute_string())
...
class HtmlText(object):
    ...
    def autoclose(self):
        ...
        for t in self.tokens:
            if self.__is_open(t) and not t.self_closes:
                close_stack.append(t)
    ...
    def append(self, token):
        if self.__is_close(token):
            for prev in reversed(self.tokens):
                if (
                    self.__is_open(prev)
                    and prev.closed_by is None
                    and prev.tag == token.tag
                ):
                    token.closes = prev
            ...
            for i in range(-1, -len(self.tokens), -1):
                ...
                if (
                    is_close
                    and last.tag == token.tag
                    and last.closes.coalesce
                    and last.closes.attributes == token.attributes
                ):
            ...
            if len(tokens) > 0:
                if len(self.tokens) > 0:
                    ...
                    if is_close and self.__is_open(next):
                        closed = last.closes
                        if (
                            last.tag == next_token.tag
                            and closed.coalesce
                            and closed.attributes == next.attributes
                        ):
    ...
    def to_html(self):
        ...
        for token in self.tokens:
            if isinstance(token, OpenTag):
                ...
                if token.tag not in inline_tags__:
                    result += "\n"
                result += token.open()

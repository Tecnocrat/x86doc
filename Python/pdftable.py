...
class Rect(object):
    ...
    def height(self):
        return abs(self.__y1 - self.__y2)
    ...
    def __repr__(self):
        ...
        return "Rect%s(%0.2f,%0.2f,%0.2f,%0.2f)" % (
            orientation,
            self.x1(),
            self.y1(),
            self.x2(),
...
def sort_rect(a, b):
    ydiff = a.y1() - b.y1()
    if abs(ydiff) > 0.7:
        return int(ydiff * 100)

    xdiff = a.x1() - b.x1()
    if abs(xdiff) > 0.7:
        return int(xdiff * 100)
...
class Table(TableBase):
    def __init__(self, group):
        ...
        if len(self.__columns) > 2:
            ...
            missingC.sort(
                key=sort_rect_by_position(Rect.y1, Rect.xmid, self.__columns[-1])
            )
            for missing in missingC:
                ...
                for i in range(beginIndex, endIndex):
                    self.__data_layout[i][rightColumn] = self.__data_layout[i][
                        leftColumn
                    ]
        ...
        if len(self.__rows) > 2:
            ...
            missingR.sort(
                key=sort_rect_by_position(Rect.x1, Rect.ymid, self.__rows[-1])
            )
            for missing in missingR:
                ...
                if beginIndex > 0:
                    prev = beginIndex - 1
                    if (
                        self.__data_layout[topRow][prev]
                        == self.__data_layout[topRow][beginIndex]
                    ):
                        continue
                ...
                if endIndex < len(self.__rows) - 1:
                    ...
                    if (
                        self.__data_layout[topRow][prev]
                        == self.__data_layout[topRow][endIndex]
                    ):
                        continue
    ...
    def columns(self):
        return len(self.__columns) - 1

    def item_count(self):
        count = 0
        for cell in self.__data_storage:
            count += len(cell)
        return count

    def bounds(self):
        return Rect(
            self.__columns[0], self.__rows[0], self.__columns[-1], self.__rows[-1]
        )
    ...
    def __identify_missing_col_lines(self, vertical):
        sort_key = sort_rect_by_position(
            Rect.y1, Rect.xmid, self.__rows[0] - self.__rows[-1]
        )
    ...
    def __identify_missing_row_lines(self, horizontal):
        sort_key = sort_rect_by_position(
            Rect.x1, Rect.ymid, self.__columns[-1] - self.__columns[0]
        )
    ...
    def __dim_index(self, array, value):
        ...
        raise Exception(
            "improbable (%g between %g and %g)" % (value, array[0], array[-1])
        )
